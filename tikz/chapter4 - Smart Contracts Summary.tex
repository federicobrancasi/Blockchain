...
\begin{tikzpicture}
    ...
    \begin{scope}[mindmap,
        ...
        \node [root concept] (team) {SMART\\CONTRACTS}[rotate=202.5] % root
        child [partner, style={concept color=Orange}] { node {Automation}
        }
        child [partner, style={concept color=Blue}] { node {Multi\\Legislation}
        }
        child [partner, style={concept color=Orange}] { node {Code is Law}
        }
        child [partner, style={concept color=Blue}] { node {Usability}
        }
        % child [partner, style={concept color=Orange}] { node {New\\Business\\Models}
        % }
        child [partner, style={concept color=Orange}] { node {Foreseeable}
        }
        child [partner, style={concept color=Blue}] { node {Skills}
        }
        child [partner, style={concept color=Orange}] { node {Risk Reduction}
        }
        child [partner, style={concept color=Blue}] { node {Flexibility}
        };
    \end{scope}
    ...
    \begin{scope}[xshift=4.5cm, yshift=-6cm,every node/.style={align=left,text=black}]
        ...
        \node{Against of}; & \fill [Blue!80] (0,.25ex) circle (1ex);\\
        \node{In Favour of}; & \fill [Orange] (0,.25ex) circle (1ex); \\
        };
    \end{scope}
\end{tikzpicture}
